...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            login(request, user)
            request.session['token'] = MasterPassOperator.master_pass_recorder(form['username'] + form['password1'])
...
def c_pass_extract(request):
    ...
    new_token, crypt_pass = MasterPassOperator.master_pass_extractor(token)
